...
public:
    ...
    Table(string tableName, vector<string> columns, bool isMatrix);
    bool load();
    ...
    void transpose();
    void isSymmetric();
    void transposeHelper(int newValue, int rowCoordinate, int columnCoordinate);

    // Returns name of a huge sorted tempFile of size (no. of pages * pageSize) i.e. all pages are in the huge file
    string sortTable(vector<string> attributesToSort, vector<SortingStrategy> strategies); 
    ...
    void writeRow(vector<T> row, ostream &fout)
    {
        ...
        for (int columnCounter = 0; columnCounter < min(20, (int)row.size()); columnCounter++)
        {
            if (columnCounter != 0)
                fout << ", ";
