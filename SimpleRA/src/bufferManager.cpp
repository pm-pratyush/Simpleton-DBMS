...
/**
 * @brief Function that removes a page from the pool.
 * 
 * @param p 
 */
void BufferManager::updatePage(Page p)
{
    ...
    deque<Page> pagesCopy = this->pages;
    this->pages.clear();

    for(auto page : pagesCopy)
    {
        if(page.pageName != p.pageName)
...
Page BufferManager::readTempPage(string tableName, int pageIndex)
{
    logger.log("BufferManager::readTempPage");
    string pageName = "../data/temp/" + tableName + "_Page" + to_string(pageIndex);
    ...
    Page page(tableName, pageIndex, rows, rows.size());
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
